...
export async function handleFetchEmprendimiento(id: string): Promise<Emprendedor> {
  try {
     ...
     const response = await fetch(`https://apidescubriendoecoempresarias-gybugkhkbagse2e4.canadacentral-01.azurewebsites.net/api/emprendimientos/Obtener?${parameters.toString()}`);
