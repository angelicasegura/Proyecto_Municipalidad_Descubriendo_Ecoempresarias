...
import { actualizarEstadoLugar } from "./actions/actualizarEstadoLugar";
...
export default function AdminLugaresPage() {
  ...
  const actualizarEstadoMutation = useMutation({
    mutationFn: ({ id, estado }: { id: number; estado: number }) =>
      actualizarEstadoLugar(id, estado),
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 mb-20">
      <div className="flex justify-between mb-6">
        <h1 className="text-2xl font-bold">Administración de Lugares</h1>
        <Link
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y">
            {lugares.map((l: any, index: number) => {
              const estadoActivo =
                l.nombreEstado === "Activo" || l.nombreEstado === "ACTIVO";
              return (
      ...
      {lugarAInactivar && (
        <ModalConfirmar
          titulo={
            lugarAInactivar.nombreEstado === "Activo"
              ? "Inactivar Lugar"
              : "Activar Lugar"
          }
          mensaje={
            lugarAInactivar.nombreEstado === "Activo"
              ? `¿Estás seguro que deseas inactivar el lugar "${lugarAInactivar.nombre}"?`
              : `¿Estás seguro que deseas activar el lugar "${lugarAInactivar.nombre}"?`
          }
          textoBoton={
            lugarAInactivar.nombreEstado === "Activo"
              ? "Sí, inactivar"
          ...
          colorBoton={
            lugarAInactivar.nombreEstado === "Activo" ? "red" : "green"
          }
          onConfirmar={() =>
            actualizarEstadoMutation.mutate({
              id: lugarAInactivar.lugar_id,
              estado: lugarAInactivar.nombreEstado === "Activo" ? 0 : 1,
            })
          }
